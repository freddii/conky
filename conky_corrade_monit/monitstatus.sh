...
QUERY_STRING=$(curl -d "command=execute&group=$GroupUUID&password=$GroupPassword&file=/usr/bin/sudo&parameter= /usr/bin/monit status corrade$BotFolderName" $HttpServer)
arr=(${QUERY_STRING//[=&]/ })
