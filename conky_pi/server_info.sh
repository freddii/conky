...
#${color1}LastUpdate: ${color2}${execi 10 ./server_info.sh -s pi LastUpdate}
function last_update {
pathConkyx="$( cd "$(dirname "$0")" ; pwd -P )";
fileNamex=$(ls -1At ${pathConkyx}/log/$server-* 2> /dev/null | head -1);
LastUpdate=$(( `date +%s` - `stat -L --format %Y $fileNamex` ));
echo "$LastUpdate" " sec"
}
